\documentclass[12pt,a4paper,twoside,openright]{report}
	...
	The blockchain, in its simplest form, is a series of blocks of data, where each block contains the cryptographic hash of the previous block in the chain. The chain is allowed to exhibit arbitrary branching, although in most applications, branches which are significantly shorter than the longest branch are disregarded as being incorrect. Figure \ref{fig:mainblockchain} is a graphical representation of a typical blockchain data structure.\\
	\\
	The significance of blocks containing the hash of the previous block, is that if a change were to be made to a previous block, it would change the hashes of all subsequent blocks in the chain. This means that once a block is committed to the blockchain, it cannot be modified, without changing the entire history of the blockchain from that moment onwards.
	\begin{figure}
	...
	We will explore the use of the Git as a blockchain during the Implementation chapter.\\
	\\
	...
	\\
	However, whilst it is possible to think of applications for blockchain technology in almost every sector, the development of applications outside the scope of crytocurrencies has been limited. 
	The reason for this is still very much open to debate and the subject of a lot of controversy, however, one argument is that it can be hard for developers to build applications that leverage the power of the blockchain as there are few services or libraries to help with this.\\
	\\
	If one considers the example of OCaml, there are no libraries which allow a user to easily get started with building blockchain applications. The aim of this project 
	\chapter{Preparation}
